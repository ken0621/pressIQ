...
class Payroll
{
	...
	public static function getdeductionv2($employee_id = 0, $date_start = '0000-00-00', $date_end = '0000-00-00', $period = '', $payroll_period_category = '', $shop_id = 0, $month_contri = '', $period_count = '')
	{
		...
		else
		{
			if($shop_id == 89)
			{
				$month[0] = date('Y-m-02', strtotime($date_end));
			}
			else
			{
		...
		if($period_count == 'first_period')
		{
			$period = 'First Period';
		}
		else if($period_count == 'middle_period')
		{
		...
		foreach($_deduction as $deduction)
		{
			...
			if($month_contri != 'March' && $month_contri != 'February')
			{
				$month_contrib = $month_contri;
			}
			...
			$payroll_month_payment_amount = Tbl_payroll_deduction_payment_v2::getmonthdeductionpayment($employee_id, $temp['payroll_deduction_id'], $temp['deduction_name'], $month, $month_contrib)->first();
			...
			/* if balance is less than to periodical deduction */
			if($deduction->balance_deduction < $temp['payroll_periodal_deduction'])
			{
				$temp['payroll_periodal_deduction'] = $deduction->balance_deduction;
			}	
