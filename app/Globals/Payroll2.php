...
class Payroll2
{
	public static function get_contribution_information_for_a_month($shop_id, $month, $year)
	{
		...
		foreach($data["_employee"] as $key => $employee)
		{
				...
				if($total_pagibig_ee == 0)
				{
					$data["pagibig"] = Tbl_payroll_period::GetContributions_employee($shop_id, $month, $year,$employee->employee_id)->get();
					foreach($data["pagibig"] as $key => $pagibig)
					{
						if($pagibig->pagibig_ee != 0)
						{
							$_contribution[$employee->employee_id]->total_pagibig_ee = $pagibig->pagibig_ee;
							$_contribution[$employee->employee_id]->total_pagibig_er = $pagibig->pagibig_er;
							$_contribution[$employee->employee_id]->total_pagibig_ee_er = $pagibig->pagibig_ee + $pagibig->pagibig_er;
						}
  ...
  //       }
	...
	public static function cutoff_breakdown_deductions($return, $data)
	{
		...
		$deduction = Payroll::getdeductionv2($employee_id, $start_date, $end_date, $period_category_arr['period_category'], $period_category, $shop_id, $period_info['month_contribution'], $period_info['period_count']);
	...
	{	
		...
		{	
			...
			{	
					...
					$total_allowance       = $employee->allowance + $employee->adjustment_allowance;
					$total_bonus           = $employee->adjustment_bonus;
					$total_commission      = $employee->adjustment_commission;
					$total_incentives      = $employee->adjustment_incentives;
					$total_additions       = $employee->adjustment_additions;
					$total_deminimis       = $employee->allowance_de_minimis + $employee->adjustment_de_minimis_benefit;
					$total_13_month        = $employee->adjustment_13th_month_and_other;
			...
			$employee_total[$employee->employee_id]->bonus_total  		           = $total_bonus;
			$employee_total[$employee->employee_id]->commission_total  		       = $total_commission;
			$employee_total[$employee->employee_id]->incentives_total  		       = $total_incentives;
			$employee_total[$employee->employee_id]->additions_total  		       = $total_additions;
			$employee_total[$employee->employee_id]->deminimis_total  		       = $total_deminimis;
			$employee_total[$employee->employee_id]->month_total_13  		       = $total_13_month;
