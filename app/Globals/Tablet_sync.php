    	...
    	if($data_id)
    	{
    		...
    		if($tablet_data)
    		{
    			...
    			foreach ($all_inv as $key => $value) 
    			{
			        ...
			        $invoice_info['new_inv_id']         = $sir_id."-".$value['inv']['new_invoice_id'];
					...
					$total_info                         = [];
					$total_info['total_subtotal_price'] = $value['inv']['subtotal_price'];
					...
					$total_info['total_overall_price']  = $value['inv']['overall_price'];

					$item_info                          = [];
					$_itemline                          = $value[$key]['invline'];
					...
					{
						if($value[$key]['inv']['credit_memo_id'] == $key_cm)
						{
            				$cm_item_info = [];
					...
					foreach ($all_manual_inv as $key_manual => $manual_inv) 
					{
						...
						{
							$ins_manual['sir_id'] = $sir_id;
							$ins_manual['inv_id'] = $inv_id;
							$ins_manual['manual_invoice_date'] = $manual_inv['manual_invoice_date'];
							$ins_manual['created_at'] = $manual_inv['created_at'];

							Tbl_manual_invoice::insert($ins_manual);
    			...
    			foreach ($all_cm as $key_cm => $value_cm)
				{
	            	...
	            	foreach ($all_manual_cm as $key_manual => $manual_cm) 
					{
						...
						{
							$ins_manual['sir_id'] = $sir_id;
							$ins_manual['cm_id'] = $cm_id;
							$ins_manual['manual_cm_date'] = $manual_cm['manual_cm_date'];
							$ins_manual['created_at'] = $manual_cm['created_at'];

							Tbl_manual_credit_memo::insert($ins_manual);
				...
				{  
			        ...
			        foreach ($all_manual_rp as $key_manual => $manual_rp) 
					{
						...
						{
							$ins_manual['sir_id'] = $sir_id;
							$ins_manual['agent_id'] = $manual_rp['agent_id'];
							$ins_manual['rp_id'] = $rcvpayment_id;
							$ins_manual['rp_date'] = $manual_rp['rp_date'];
							$ins_manual['created_at'] = $manual_rp['created_at'];

							Tbl_manual_receive_payment::insert($ins_manual);
