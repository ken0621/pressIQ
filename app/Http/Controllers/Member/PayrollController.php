...
class PayrollController extends Member
{
	...
	public function modal_employee_update()
	{
     ...
     public function generate_payslip_v2($id)
     {          
          $payslip  = Tbl_payroll_payslip::payslip(Self::shop_id())->first();
     ...
     public function payslip_template_group_ptkab($time_keeping_approve_id)
     {    
          $_temp_ptkab        = array();
          $_type              = Tbl_payroll_time_keeping_approved_breakdown::select(DB::raw('distinct(ptkab_type)'))
                                   ->where('time_keeping_approve_id', $time_keeping_approve_id)
                                   ->get();   
          foreach ($_type as $type) 
          {
               $_record       = Tbl_payroll_time_keeping_approved_breakdown::where('time_keeping_approve_id', $time_keeping_approve_id)
                                   ->where('ptkab_type', $type->ptkab_type)
                                   ->get();
               
               $temp     = array();
               foreach ($_record as $key => $record) 
               {
                    $temp_ptkab['ptkab_label']     = $record->ptkab_label;
                    $temp_ptkab['ptkab_amount']    = number_format($record->ptkab_amount, 2, '.', ',');  
                    array_push($temp, $temp_ptkab);                                
               } 
               $_temp_ptkab[$type->ptkab_type]   = $temp;         
          }
          return $_temp_ptkab;
     }
