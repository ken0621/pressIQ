...
class ProductOrderController2 extends Member
{
    ...
    public function table()
    {
        ...
        if($active_tab == "paid")
        {
            $data["_raw_table"] = Transaction::get_transaction_list($shop_id, 'receipt',$keyword);
        }
        elseif($active_tab == "unconfirmed")
        {
            $data["_raw_table"] = Transaction::get_transaction_list($shop_id, 'proof',$keyword);
        }
        elseif($active_tab == "pending")
        {
            $data["_raw_table"] = Transaction::get_transaction_list($shop_id, 'order',$keyword);
        }
        elseif($active_tab == "reject")
        {
            $data["_raw_table"] = Transaction::get_transaction_list($shop_id, 'reject',$keyword);
        }
        else
        {
            $data["_raw_table"] = Transaction::get_transaction_list($shop_id, 'receipt',$keyword);
