...
function accountJs()
{
    ...
    function add_event_active_membership()
    {
       $('.account_table').on('click', '.membershi', function(e)
       {
        	$product_id = $(e.currentTarget).closest("i").attr("acc_id");
            if($(e.currentTarget).hasClass("active"))
            {
                $(e.currentTarget).removeClass("active");
                set_membership($product_id, 1);
            }
            else
            {
                $(e.currentTarget).addClass("active");    
                set_membership($product_id, 0);
            }
       });
    }
    function set_membership($acc, $value)
    {
        $.ajax(
        {
            url:"admin/maintenance/accounts/disable_membership",
            dataType:"json",
    ...
    function set_product($acc, $value)
    {
        $.ajax(
        {
            url:"admin/maintenance/accounts/disable_product",
            dataType:"json",
            data:{ "id":$acc, "value": $value, "_token": $(".token").val() },
            type:"post",
            success: function(data)
