        ...
        @if(count($_receive_payment) > 0)
            @foreach($_receive_payment as $rp)
                <tr>
                    ...
                    <td class="text-center">{{$rp->transaction_refnum != "" ? $rp->transaction_refnum : $rp->est_id}}</td>
                    <td class="text-center">{{date('F d, Y',strtotime($rp->est_date))}}</td>
                    <td class="text-center">{{currency('',$rp->est_overall_price)}}</td>
