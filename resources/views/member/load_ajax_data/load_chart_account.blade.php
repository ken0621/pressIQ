@foreach($_account as $key=>$account)
	<option value="{{$account['account_id']}}" indent="{{$account['account_sublevel']}}" add-search="{{$add_search}}" reference="{{$account['account_type'] == 'Accounts Receivable' ? 'customer' : ($account['account_type'] == 'Accounts Payable' ? 'vendor' : '')}}"
