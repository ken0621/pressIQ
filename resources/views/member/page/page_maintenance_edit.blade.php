		...
		<div style="margin-top: 15px;">
			...
			<div class="form-group {{ $fields->type == "hidden" ? "hide" : "" }}">
				...
				@elseif($fields->type == "image")
					<input value="{{ $edit[$fields->name] }}" type="hidden" name="{{ $fields->name }}" class="maintenance-image-input" key="{{ $key }}-{{ $fields->type }}-{{ $fields->name }}">
					<div class="maintenance-image-holder" key="{{ $key }}-{{ $fields->type }}-{{ $fields->name }}">
				...
				@elseif($fields->type == "map")
					...
					<script>
				      ...
				      function initialize() 
				      {
				        var bangalore = { lat: {{ $edit["latitude"] }}, lng: {{ $edit["longitude"] }} };
				...
				@else
					<input step="any" class="form-control" type="{{ $fields->type }}" name="{{ $fields->name }}" value="{{ $edit[$fields->name] }}">
				@endif
			</div>
			@endforeach
		</div>
