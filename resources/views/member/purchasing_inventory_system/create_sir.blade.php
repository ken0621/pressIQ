...
@section('content')
<form class="global-submit form-to-submit-transfer" role="form" action="/member/pis/sir/create_submit" method="POST" >
    ...
    <div class="panel panel-default panel-block panel-title-block" id="top">
        <div class="panel-heading">
            <div>
                ...
                <h1>
                    <span class="page-title">Load Out Form &raquo; Create</span>
                    <small>
                    	Create Load out Form
                    </small>
                </h1>
                    ...
                    <select class="form-control select-truck" name="truck_id">
                        @if($_truck)
                            @foreach($_truck as $truck)
                                <option value="{{$truck->truck_id}}">{{$truck->plate_number}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                    ...
                    <select class="form-control select-employee" name="sales_agent_id">
                        @if($_employees)
                            @foreach($_employees as $employee)
                                <option value="{{$employee->employee_id}}">{{$employee->first_name}} {{$employee->middle_name}} {{$employee->last_name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>            
