@if(isset($_invoice) && $_invoice != null)
	@foreach($_invoice as $invoice)
		<tr>
		    ...
		    <td><input type="text" class="text-right original-amount" value="{{number_format($invoice['inv_overall_price'],5) }}" disabled /></td>
		    <td><input type="text" class="text-right balance-due" value="{{number_format(($invoice['inv_overall_price']) - $invoice['amount_applied'] + (isset($invoice['rpline_amount']) ? $invoice['rpline_amount'] : 0 ),5)}}" disabled /></td>
