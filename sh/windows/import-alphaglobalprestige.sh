...
username="root";
password="";
database="alphaglobalprestige";

## LOGIN SSH ##
rm database/digimahouse_latest_backup.sql.gz
rm database/digimahouse_latest_backup.sql

plink -pw "digima2018" digima@159.89.201.203 "~/export.sh"

## DATABASE ##
mysql -u$username -e "DROP DATABASE $database"
mysql -u$username -e "CREATE DATABASE $database"

curl --insecure "http://alpha-globalprestige.com/digimahouse_latest_backup.sql.gz" -o database/digimahouse_latest_backup.sql.gz
